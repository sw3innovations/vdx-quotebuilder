...
import { adminApi } from "@/api/apiBackend";
...
export default function Dashboard() {
  ...
  const stats = React.useMemo(() => {
    const ag = statsData.aguardando_aprovacao || 0;
    const ap = statsData.aguardando_pagamento || 0;
    const ep = statsData.em_producao          || 0;
    const ar = statsData.aguardando_retirada  || 0;
    const ca = statsData.cancelado            || 0;

    return {
      total:       ag + ap + ep + ar + ca,
      emAndamento: ag + ap + ep,
      prontos:     ar,
      valorTotal:  orcamentos
        .filter(o => o.status !== 'cancelado')
        .reduce((sum, o) => sum + (o.preco_total || 0), 0),
    };
  }, [statsData, orcamentos]);

  const statCards = [
    { label: "Total Orçamentos", value: stats.total,       icon: <FileText className="w-6 h-6 text-blue-600" />,     bg: "bg-blue-100" },
    { label: "Em Andamento",     value: stats.emAndamento, icon: <Clock className="w-6 h-6 text-amber-600" />,       bg: "bg-amber-100" },
    { label: "Prontos",          value: stats.prontos,     icon: <CheckCircle2 className="w-6 h-6 text-green-600" />, bg: "bg-green-100" },
    { label: "Valor Total",      value: null,              icon: <DollarSign className="w-6 h-6 text-emerald-600" />, bg: "bg-emerald-100" },
  ];
  ...
  return (
    <div className="w-full">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-8">
        {statCards.map(({ label, value, icon, bg }, i) => (
          <motion.div
            key={label}
              ...
              {loadingOrcamentos ? (
                ...
                <div className="space-y-3">
                  {orcamentos.slice(0, 5).map((orcamento) => (
                    <div key={orcamento.id} className="block">
                        ...
                        <div className="text-right">
                          ...
                          {orcamento.preco_total && (
                            <p className="text-sm font-medium text-slate-900 mt-1">
                              {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(orcamento.preco_total)}
                            </p>
                          )}
                        </div>
                      </div>
